...
func (t *Table) OnTimer() {
	...
	log.Debugf("[Stage] OnTimer timeout. St:%v Tb=%v", state, t.Desc())
	switch state {
	case StWait:
		t.checkCanStart()
	case StReady:
...
func (t *Table) onActionTimeout() {
	p := t.GetActivePlayer()
	if p == nil {
		log.Errorf("onActionTimeout: no active player at table %v", t.Desc())
		return
	}
	if !p.IsGaming() {
	...
	if err != nil {
		log.Errorf("onActionTimeout: failed to generate action req. %v %v ", t.Desc(), err)
		return
	}
...
func (t *Table) settle() *SettleObj {
	...
	if err := settle.Settle(); err != nil {
		log.Errorf("settle error: tb=%v %v", t.Desc(), err)
		return nil
	}
