...
func (s *Service) OnSessionOpen(sess *websocket.Session) {
	// log.Debugf("OnOpenFunc: %q", sess.PieceID())
}

// OnSessionClose 连接关闭回调
func (s *Service) OnSessionClose(sess *websocket.Session) {
	// log.Debugf("OnCloseFunc: %q", sess.PieceID())
...
func (s *Service) OnDiceReq(ctx context.Context, in *v1.DiceReq) (*v1.DiceRsp, error) {
	rs := s.uc.Swapper(ctx)
	if rs.Code != 0 {
		return nil, nil
	}
	rs.Table.OnDiceReq(rs.Player, in, false)
	return &v1.DiceRsp{}, nil
}

func (s *Service) OnMoveReq(ctx context.Context, in *v1.MoveReq) (*v1.MoveRsp, error) {
	rs := s.uc.Swapper(ctx)
	if rs.Code != 0 {
		return nil, nil
	}
	rs.Table.OnMoveReq(rs.Player, in, false)
	return &v1.MoveRsp{}, nil
}
