...
#!root, install for current user
	...
	cp ./cmd/kratos/kratos $${REPLY}/;cp ./cmd/protoc-gen-go-errors/protoc-gen-go-errors $${REPLY}/;cp ./cmd/protoc-gen-go-http/protoc-gen-go-http $${REPLY}/;cp ./cmd/protoc-gen-go-tcp/protoc-gen-go-tcp $${REPLY}/;else mkdir -p '$(BIN)';\
	cp ./cmd/kratos/kratos '$(BIN)';cp ./cmd/protoc-gen-go-errors/protoc-gen-go-errors '$(BIN)';cp ./cmd/protoc-gen-go-http/protoc-gen-go-http '$(BIN)';cp ./cmd/protoc-gen-go-tcp/protoc-gen-go-tcp '$(BIN)'; fi)
endif
...
.PHONY: proto
proto:
	protoc --proto_path=./api --proto_path=./third_party --go_out=paths=source_relative:./api --go-grpc_out=paths=source_relative:./api --go-http_out=paths=source_relative:./api --go-tcp_out=paths=source_relative:./api metadata/metadata.proto
